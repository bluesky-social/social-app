...
export function Root(_a) {
    ...
    var returnLocationSV = useSharedValue(null);
    ...
    var context = useMemo(function () {
        return ({
            ...
            returnLocationSV: returnLocationSV,
...
export function Trigger(_a) {
    ...
    useEffect(function () {
        if (context.isOpen && context.measurement) {
            var hide_1 = KeyboardEvents.addListener('keyboardDidHide', function () {
                measureView(ref.current, insets)
                    .then(function (newMeasurement) {
                    if (!newMeasurement || !context.measurement)
                        return;
                    if (newMeasurement.x !== context.measurement.x ||
    ...
    var doubleTapGesture = useMemo(function () {
        return Gesture.Tap()
            .numberOfTaps(2)
            .hitSlop(HITSLOP_10)
            .onEnd(function () { return void open('auxiliary-only'); })
            .runOnJS(true);
    }, [open]);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(GestureDetector, { gesture: composedGestures, children: _jsx(View, { ref: ref, style: [{ opacity: context.isOpen ? 0 : 1 }, style], children: children({
                    ...
                    }) }) }), isFocused && image && measurement && (_jsx(Portal, { children: _jsx(TriggerClone, { label: contentLabel, translation: translationSV, animation: animationSV, image: image, measurement: measurement, returnLocation: context.returnLocationSV, onDisplay: function () {
...
function TriggerClone(_a) {
    var translation = _a.translation, animation = _a.animation, image = _a.image, measurement = _a.measurement, returnLocation = _a.returnLocation, onDisplay = _a.onDisplay, label = _a.label;
    var _ = useLingui()._;
    var animatedStyles = useAnimatedStyle(function () {
        var anim = animation.get();
        var ret = returnLocation.get();
        var returnOffsetX = ret
            ? interpolate(anim, [0, 1], [ret.x - measurement.x, 0])
            : 0;
        var returnOffsetY = ret
...
function measureView(view, insets) {
    if (!view)
        return Promise.resolve(null);
    return new Promise(function (resolve) {
        view === null || view === void 0 ? void 0 : view.measureInWindow(function (x, y, width, height) {
            return resolve({
                x: x,
                y: y +
                    platform({
