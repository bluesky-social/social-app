// This file is generated by pnpm icons:generate. Do not edit.

import {Bubbles_Stroke2_Corner2_Rounded as CanonicalBubbles_Stroke2_Corner2_Rounded} from './Bubbles'
import {createSinglePathSVG} from './TEMPLATE'

export const Bubble_Filled_Stroke2_Corner2_Rounded = createSinglePathSVG({
  path: 'M19.002 3a3 3 0 0 1 3 3v10a3 3 0 0 1-3 3H12.28l-4.762 2.858A1 1 0 0 1 6.002 21v-2h-1a3 3 0 0 1-3-3V6a3 3 0 0 1 3-3h14Z',
  viewBox: '0 0 24 24',
})

export const Bubble_Stroke2_Corner2_Rounded = createSinglePathSVG({
  path: 'M2.002 6a3 3 0 0 1 3-3h14a3 3 0 0 1 3 3v10a3 3 0 0 1-3 3H12.28l-4.762 2.858A1 1 0 0 1 6.002 21v-2h-1a3 3 0 0 1-3-3V6Zm3-1a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h2a1 1 0 0 1 1 1v1.234l3.486-2.092a1 1 0 0 1 .514-.142h7a1 1 0 0 0 1-1V6a1 1 0 0 0-1-1h-14Z',
  viewBox: '0 0 24 24',
})

export const Bubble_Stroke2_Corner3_Rounded = createSinglePathSVG({
  path: 'M2.002 7a4 4 0 0 1 4-4h12a4 4 0 0 1 4 4v8a4 4 0 0 1-4 4H12.28l-4.762 2.858A1 1 0 0 1 6.002 21v-2a4 4 0 0 1-4-4V7Zm4-2a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h1a1 1 0 0 1 1 1v1.234l3.486-2.092a1 1 0 0 1 .514-.142h6a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-12Z',
  viewBox: '0 0 24 24',
})

export const BubbleInfo_Stroke2_Corner2_Rounded = createSinglePathSVG({
  path: 'M6.002 5h12a1 1 0 0 1 1 1v10.036a1 1 0 0 1-1 1h-2.626a2 2 0 0 0-1.276.46l-2.098 1.738-2.065-1.731a2 2 0 0 0-1.285-.467h-2.65a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Zm12-2h-12a3 3 0 0 0-3 3v10.036a3 3 0 0 0 3 3h2.65l2.704 2.266a1 1 0 0 0 1.28.004l2.74-2.27h2.626a3 3 0 0 0 3-3V6a3 3 0 0 0-3-3ZM13 11.75a1 1 0 1 0-2 0v2a1 1 0 1 0 2 0v-2ZM12 10a1.25 1.25 0 1 1 0-2.5 1.25 1.25 0 0 1 0 2.5Z',
  viewBox: '0 0 24 24',
})

export const BubbleQuestion_Stroke2_Corner0_Rounded = createSinglePathSVG({
  path: 'M5.002 17.036V5h14v12.036h-3.986a1 1 0 0 0-.639.23l-2.375 1.968-2.344-1.965a1 1 0 0 0-.643-.233H5.002ZM20.002 3h-16a1 1 0 0 0-1 1v14.036a1 1 0 0 0 1 1h4.65l2.704 2.266a1 1 0 0 0 1.28.004l2.74-2.27h4.626a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1Zm-7.878 3.663c-1.39 0-2.5 1.135-2.5 2.515a1 1 0 0 0 2 0c0-.294.232-.515.5-.515a.507.507 0 0 1 .489.6.2.2 0 0 1-.027.048 1.1 1.1 0 0 1-.267.226c-.508.345-1.128.923-1.286 1.978a1 1 0 1 0 1.978.297.76.76 0 0 1 .14-.359c.063-.086.155-.169.293-.262.436-.297 1.18-.885 1.18-2.013 0-1.38-1.11-2.515-2.5-2.515ZM12 15.75a1.25 1.25 0 1 1 0-2.5 1.25 1.25 0 0 1 0 2.5Z',
  viewBox: '0 0 24 24',
})

export const BubbleSmile_Stroke2_Corner2_Rounded_Large = createSinglePathSVG({
  path: 'M32 7c13.807 0 25 11.193 25 25S45.807 57 32 57c-3.638 0-7.098-.778-10.219-2.177a1.84 1.84 0 0 0-1.152-.133l-8.877 2.004c-2.655.6-5.015-1.802-4.37-4.446l2.077-8.51a1.84 1.84 0 0 0-.139-1.207A24.9 24.9 0 0 1 7 32C7 18.193 18.193 7 32 7Zm0 2C19.298 9 9 19.298 9 32c0 3.463.765 6.745 2.134 9.688.358.769.479 1.658.267 2.525l-2.076 8.51a1.667 1.667 0 0 0 1.987 2.02l8.876-2.005a3.84 3.84 0 0 1 2.412.26A22.9 22.9 0 0 0 32 55c12.703 0 23-10.297 23-23S44.703 9 32 9Zm3.138 28.215a1 1 0 0 1 1.414 1.414 6.143 6.143 0 0 1-8.688 0 1 1 0 0 1 1.414-1.414 4.143 4.143 0 0 0 5.86 0ZM20 29.667a2.333 2.333 0 1 1 0 4.666 2.333 2.333 0 0 1 0-4.666Zm24 0a2.333 2.333 0 1 1 0 4.666 2.333 2.333 0 0 1 0-4.666Z',
  viewBox: '0 0 64 64',
})

/** @deprecated Import Bubbles_Stroke2_Corner2_Rounded from `#/components/icons/Bubbles` instead. */
export const Bubbles_Stroke2_Corner2_Rounded =
  CanonicalBubbles_Stroke2_Corner2_Rounded
