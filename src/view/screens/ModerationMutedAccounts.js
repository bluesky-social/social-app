...
export function ModerationMutedAccounts(_a) {
    var _this = this;
    var _b;
    var t = useTheme();
    ...
    var _d = useMyMutedAccountsQuery(), data = _d.data, isFetching = _d.isFetching, isError = _d.isError, error = _d.error, refetch = _d.refetch, hasNextPage = _d.hasNextPage, fetchNextPage = _d.fetchNextPage, isFetchingNextPage = _d.isFetchingNextPage;
    var isEmpty = !isFetching && !((_b = data === null || data === void 0 ? void 0 : data.pages[0]) === null || _b === void 0 ? void 0 : _b.mutes.length);
    var profiles = useMemo(function () {
        if (data === null || data === void 0 ? void 0 : data.pages) {
            return data.pages.flatMap(function (page) { return page.mutes; });
        }
        return [];
    }, [data]);
    ...
    var renderItem = function (_a) {
        ...
        if (!moderationOpts)
            return null;
        return (_jsx(View, { style: [a.py_md, a.px_xl, a.border_t, t.atoms.border_contrast_low], children: _jsx(ProfileCard.Link, { profile: item, testID: "mutedAccount-".concat(index), children: _jsxs(ProfileCard.Outer, { children: [_jsxs(ProfileCard.Header, { children: [_jsx(ProfileCard.Avatar, { profile: item, moderationOpts: moderationOpts }), _jsx(ProfileCard.NameAndHandle, { profile: item, moderationOpts: moderationOpts })] }), _jsx(ProfileCard.Labels, { profile: item, moderationOpts: moderationOpts }), _jsx(ProfileCard.Description, { profile: item })] }) }) }, item.did));
    };
    return (_jsxs(Layout.Screen, { testID: "mutedAccountsScreen", children: [_jsxs(Layout.Header.Outer, { children: [_jsx(Layout.Header.BackButton, {}), _jsx(Layout.Header.Content, { children: _jsx(Layout.Header.TitleText, { children: _jsx(Trans, { children: "Muted Accounts" }) }) }), _jsx(Layout.Header.Slot, {})] }), _jsx(Layout.Center, { children: isEmpty ? (_jsxs(View, { children: [_jsx(Info, { style: [a.border_b] }), isError ? (_jsx(ErrorScreen, { title: "Oops!", message: cleanError(error), onPressTryAgain: refetch })) : (_jsx(Empty, {}))] })) : (_jsx(List, { data: profiles, keyExtractor: function (item) { return item.did; }, refreshing: isPTRing, onRefresh: onRefresh, onEndReached: onEndReached, renderItem: renderItem, initialNumToRender: 15, 
                    // FIXME(dan)
