...
export function SearchScreen(
  ...
  const onChangeText = React.useCallback(
    async (text: string) => {
      const updateSearchHistory = async (newQuery: string) => {
        if (newQuery && !searchHistory.includes(newQuery)) {
          let newHistory = [...searchHistory, newQuery]
    ...
    [setQuery, search, setSearchResults, searchHistory],
  )
  ...
  }, [query, searchHistory, setShowAutocompleteResults])
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <CenteredView sideBorders={isTabletOrDesktop}>
        {renderSearchHistory()}
      </CenteredView>
