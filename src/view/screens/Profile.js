...
function ProfileScreenInner(_a) {
    ...
    var _c = useResolveDidQuery(name), resolvedDid = _c.data, resolveError = _c.error, refetchDid = _c.refetch, isDidPending = _c.isPending;
    var _d = useProfileQuery({
        did: resolvedDid,
    }), profile = _d.data, profileError = _d.error, refetchProfile = _d.refetch, isPlaceholderProfile = _d.isPlaceholderData, isProfilePending = _d.isPending;
...
function ProfileScreenLoaded(_a) {
    ...
    var _m = useState(0), currentPage = _m[0], setCurrentPage = _m[1];
    var _ = useLingui()._;
    var _o = useState(null), scrollViewTag = _o[0], setScrollViewTag = _o[1];
    var postsSectionRef = useRef(null);
    var repliesSectionRef = useRef(null);
    var mediaSectionRef = useRef(null);
    var videosSectionRef = useRef(null);
    var likesSectionRef = useRef(null);
    var feedsSectionRef = useRef(null);
    var listsSectionRef = useRef(null);
    var starterPacksSectionRef = useRef(null);
    var labelsSectionRef = useRef(null);
    ...
    var onPressCompose = function () {
        var mention = profile.handle === (currentAccount === null || currentAccount === void 0 ? void 0 : currentAccount.handle) ||
            ...
            ? undefined
            : profile.handle;
        openComposer({ mention: mention, logContext: 'ProfileFeed' });
    };
                        ...
                        ? function (_a) {
                            ...
                            return (_jsx(ProfileFeedSection, { ref: postsSectionRef, feed: "author|".concat(profile.did, "|posts_and_author_threads"), headerHeight: headerHeight, isFocused: isFocused, scrollElRef: scrollElRef, ignoreFilterFor: profile.did, setScrollViewTag: setScrollViewTag, emptyStateMessage: _(msg(templateObject_14 || (templateObject_14 = __makeTemplateObject(["No posts yet"], ["No posts yet"])))), emptyStateButton: isMe
                                    ? {
                                        ...
                                        onPress: function () {
                                            return openComposer({ logContext: 'ProfileFeed' });
                                        },
                                        size: 'small',
                                        color: 'primary',
                                    }
...
function useRichText(text) {
    var agent = useAgent();
    var _a = useState(text), prevText = _a[0], setPrevText = _a[1];
    var _b = useState(function () { return new RichTextAPI({ text: text }); }), rawRT = _b[0], setRawRT = _b[1];
    var _c = useState(null), resolvedRT = _c[0], setResolvedRT = _c[1];
    if (text !== prevText) {
        setPrevText(text);
