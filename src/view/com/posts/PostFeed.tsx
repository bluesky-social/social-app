        ...
        if (isVideoFeed) {
          ...
          for (const page of data.pages) {
            for (const slice of page.slices) {
              ...
              if (
                item &&
                bsky.isType(AppBskyEmbedVideo.view, item.post.embed) &&
  ...
  const onItemSeen = useCallback(
    (item: FeedRow) => {
      ...
      const onPostSeen = (post: AppBskyFeedDefs.PostView) => {
        ...
        if (
          bsky.isType(AppBskyEmbedExternal.view, post.embed) &&
        ...
        if (
          bsky.isType(AppBskyEmbedImages.view, post.embed) ||
          bsky.isType(AppBskyEmbedGallery.view, post.embed)
        ) {
          const totalImages = bsky.isType(AppBskyEmbedGallery.view, post.embed)
            ? post.embed.items.filter(item =>
                bsky.isType(AppBskyEmbedGallery.viewImage, item),
              ).length
