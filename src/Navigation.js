...
function HomeTabNavigator() {
    var t = useTheme();
    var BLURRED_SCROLL_EDGE_EFFECT = IS_LIQUID_GLASS
        ? {
            headerShown: true,
        ...
        : {};
    return (_jsxs(HomeTab.Navigator, { screenOptions: screenOptions(t), initialRouteName: "Home", children: [_jsx(HomeTab.Screen, { name: "Home", getComponent: function () { return HomeScreen; }, options: BLURRED_SCROLL_EDGE_EFFECT }), _jsx(HomeTab.Screen, { name: "Start", getComponent: function () { return HomeScreen; }, options: BLURRED_SCROLL_EDGE_EFFECT }), commonScreens(HomeTab)] }));
}
...
function RoutesContainer(_a) {
    ...
    var linkingUrl = Linking.useLinkingURL();
    /**
    ...
    function handlePushNotificationEntry() {
        ...
        if (linkingUrl)
            return;
        var notificationResponse = Notifications.getLastNotificationResponse();
        if (notificationResponse) {
            ...
            try {
                ...
                notyLogger.error("handlePushNotificationEntry: error clearing notification response", { error: error });
            }
            var payload = getNotificationPayload(notificationResponse.notification);
            if (payload) {
                ...
                if (payload.reason === 'chat-message') {
                    ...
                    var path = notificationToURL(payload);
                    if (path === '/notifications') {
                        resetToTab('NotificationsTab');
                        notyLogger.debug("handlePushNotificationEntry: default navigate");
                    }
                    else if (path) {
                        var _a = router.matchPath(path), screen_1 = _a[0], params = _a[1];
                        // @ts-expect-error nested navigators aren't typed -sfn
