...
export default function HashtagScreen(_a) {
    var route = _a.route;
    var _b = route.params, tag = _b.tag, author = _b.author;
    var _ = useLingui()._;
    var decodedTag = React.useMemo(function () {
        return decodeURIComponent(tag);
    }, [tag]);
    var isCashtag = decodedTag.startsWith('$');
    var fullTag = React.useMemo(function () {
        // Cashtags already include the $ prefix, hashtags need # added
        return isCashtag ? decodedTag : "#".concat(decodedTag);
    }, [decodedTag, isCashtag]);
    var headerTitle = React.useMemo(function () {
        // Keep cashtags uppercase, lowercase hashtags
        var displayTag = isCashtag ? fullTag.toUpperCase() : fullTag.toLowerCase();
        return enforceLen(displayTag, 24, true, 'middle');
    }, [fullTag, isCashtag]);
    var sanitizedAuthor = React.useMemo(function () {
        if (!author)
            return;
    ...
    var onShare = React.useCallback(function () {
        ...
        url.pathname = "/hashtag/".concat(decodeURIComponent(tag));
        if (author) {
...
function HashtagScreenTab(_a) {
    var _this = this;
    var fullTag = _a.fullTag, author = _a.author, sort = _a.sort, active = _a.active;
    var _ = useLingui()._;
    ...
    var trackPostView = usePostViewTracking('Hashtag');
    var isCashtag = fullTag.startsWith('$');
    var queryParam = React.useMemo(function () {
        // Cashtags need # prefix for search: "#$BTC" or "#$BTC from:author"
        var searchTag = isCashtag ? "#".concat(fullTag) : fullTag;
        if (!author)
            return searchTag;
        return "".concat(searchTag, " from:").concat(author);
    }, [fullTag, author, isCashtag]);
    var _c = useSearchPostsQuery({ query: queryParam, sort: sort, enabled: active }), data = _c.data, isFetched = _c.isFetched, isFetchingNextPage = _c.isFetchingNextPage, isLoading = _c.isLoading, isError = _c.isError, error = _c.error, refetch = _c.refetch, fetchNextPage = _c.fetchNextPage, hasNextPage = _c.hasNextPage;
