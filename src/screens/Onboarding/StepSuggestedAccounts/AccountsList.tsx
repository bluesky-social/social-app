...
import {type useSuggestedOnboardingUsers} from '#/screens/Search/util/useSuggestedOnboardingUsers'
import {type useSuggestedUsers} from '#/screens/Search/util/useSuggestedUsers'
...
type Source = 'SuggestedUsers' | 'SuggestedOnboardingUsers'

interface Props {
  queryResult: ReturnType<
    typeof useSuggestedUsers | typeof useSuggestedOnboardingUsers
  >
  source: Source
}

export function AccountsList({queryResult, source}: Props) {
  const {
    data: suggestedUsers,
    isLoading,
    error,
  ...
  const onProfileSeen = useCallback(
    ...
    [ax, selectedInterest, source, suggestedUsers?.recId],
  )
