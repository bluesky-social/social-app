...
describe('dateDiff', () => {
  it(`works with numbers`, () => {
    expect(dateDiff(subDays(base, 3), Number(base))).toEqual('3d')
  })
  it(`works with strings`, () => {
    expect(dateDiff(subDays(base, 3), base.toString())).toEqual('3d')
  })
  it(`works with dates`, () => {
    expect(dateDiff(subDays(base, 3), base)).toEqual('3d')
  })

  it(`equal values return now`, () => {
    expect(dateDiff(base, base)).toEqual('now')
  })
  it(`future dates return now`, () => {
    expect(dateDiff(addDays(base, 3), base)).toEqual('now')
  })

  it(`values < 5 seconds ago return now`, () => {
    const then = subSeconds(base, 4)
    expect(dateDiff(then, base)).toEqual('now')
  })
  it(`values >= 5 seconds ago return seconds`, () => {
    const then = subSeconds(base, 5)
    expect(dateDiff(then, base)).toEqual('5s')
  })

  it(`values < 1 min return seconds`, () => {
    const then = subSeconds(base, 59)
    expect(dateDiff(then, base)).toEqual('59s')
  })
  it(`values >= 1 min return minutes`, () => {
    const then = subSeconds(base, 60)
    expect(dateDiff(then, base)).toEqual('1m')
  })
  ...
  it(`values < 1 hour return minutes`, () => {
    ...
    expect(dateDiff(then, base)).toEqual('59m')
  })
  it(`values >= 1 hour return hours`, () => {
    const then = subMinutes(base, 60)
    expect(dateDiff(then, base)).toEqual('1h')
  })
  it(`hours round down`, () => {
  ...
  it(`values < 1 day return hours`, () => {
    ...
    expect(dateDiff(then, base)).toEqual('23h')
  })
  it(`values >= 1 day return days`, () => {
    const then = subHours(base, 24)
    expect(dateDiff(then, base)).toEqual('1d')
  })
  it(`days round down`, () => {
  ...
  it(`values < 30 days return days`, () => {
    ...
    expect(dateDiff(then, base)).toEqual('29d')
  })
  it(`values >= 30 days return months`, () => {
    const then = subDays(base, 30)
    expect(dateDiff(then, base)).toEqual('1mo')
  })
  ...
  it(`values are rounded by increments of 30`, () => {
    ...
    expect(dateDiff(then, base)).toEqual('2mo')
  })

  it(`values < 360 days return months`, () => {
    const then = subDays(base, 359)
    expect(dateDiff(then, base)).toEqual('11mo')
  })
  it(`values >= 360 days return the earlier value`, () => {
    const then = subDays(base, 361)
    expect(dateDiff(then, base)).toEqual(then.toLocaleDateString())
  })
})
