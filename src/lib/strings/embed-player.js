...
export function parseEmbedPlayerFromUrl(url) {
    ...
    var bandcampRegex = /^[a-z\d][a-z\d-]{2,}[a-z\d]\.bandcamp\.com$/i;
    if (bandcampRegex.test(urlp.hostname)) {
        var pathComponents = urlp.pathname.split('/');
        switch (pathComponents[1]) {
            case 'album':
                return {
                    type: 'bandcamp_album',
                    source: 'bandcamp',
                    playerUri: "https://bandcamp.com/EmbeddedPlayer/url=".concat(encodeURIComponent(urlp.href), "/size=large/bgcol=ffffff/linkcol=0687f5/minimal=true/transparent=true/"),
                };
            case 'track':
                return {
                    type: 'bandcamp_track',
