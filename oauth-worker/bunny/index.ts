 ...
 *   CLIENT_ID          (Environment Variable) exact iss/sub, e.g.
 *                      https://<domain>/oauth-client-metadata.json
 *   ALLOWED_ORIGIN     (Environment Variable) exact browser Origin, e.g.
 *                      https://<domain>
 *
 ...
 *
 * Given that, this is a STRICT RECONSTRUCTING MINTER (not a signer): it never
 * signs caller-supplied JWT material. It validates the request, then builds the
 * header and payload itself from a fixed whitelist (alg/kid/typ;
 * iss/sub/aud/jti/iat/exp) so no extra header params (crit/jku/x5u/jwk/cnf) or
 * extra claims can ever be injected into a production-key signature.
 *
