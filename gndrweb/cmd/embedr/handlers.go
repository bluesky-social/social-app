...
func (srv *Server) getGanderPost(ctx context.Context, did syntax.DID, rkey syntax.RecordKey) (*appgndr.FeedDefs_PostView, error) {
...
func (srv *Server) parseGanderURL(ctx context.Context, raw string) (*syntax.ATURI, error) {
	...
	// then try gndr.app post URL
	u, err := url.Parse(raw)
	if err != nil {
		return nil, err
	}
	if u.Hostname() != "gndr.app" {
		return nil, fmt.Errorf("only gndr.app URLs currently supported")
	}
	...
	if len(pathParts) != 5 || pathParts[1] != "profile" || pathParts[3] != "post" {
		return nil, fmt.Errorf("only gndr.app post URLs currently supported")
	}
...
func (srv *Server) WebOEmbed(c echo.Context) error {
	...
	aturi, err := srv.parseGanderURL(c.Request().Context(), c.QueryParam("url"))
	if err != nil {
	...
	post, err := srv.getGanderPost(c.Request().Context(), did, aturi.RecordKey())
	...
	data := OEmbedResponse{
		...
		AuthorURL:    fmt.Sprintf("https://gndr.app/profile/%s", post.Author.Handle),
		ProviderName: "Gander Social",
		ProviderURL:  "https://gndr.app",
		CacheAge:     86400,
		Width:        &width,
		Height:       nil,
		HTML:         html,
...
func (srv *Server) WebPostEmbed(c echo.Context) error {
	...
	/*
		postView, err := srv.getGanderPost(ctx, did, rkey)
		if err == ErrPostNotFound {
			return c.String(http.StatusNotFound, fmt.Sprintf("%v", err))
		} else if err == ErrPostNotPublic {
			return c.String(http.StatusForbidden, fmt.Sprintf("%v", err))
