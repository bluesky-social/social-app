...
func (srv *Server) postEmbedHTML(postView *appgndr.FeedDefs_PostView) (string, error) {
	...
	data := struct {
		...
		ProfileURL:    template.URL(fmt.Sprintf("https://gndr.app/profile/%s?ref_src=embed", aturi.Authority())),
		PostURL:       template.URL(fmt.Sprintf("https://gndr.app/profile/%s/post/%s?ref_src=embed", aturi.Authority(), aturi.RecordKey())),
		WidgetURL:     template.URL("https://embed.gndr.app/static/embed.js"),
	}
