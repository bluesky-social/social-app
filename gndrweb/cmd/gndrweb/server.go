...
func serve(cctx *cli.Context) error {
	...
	e.Use(middleware.RateLimiterWithConfig(middleware.RateLimiterConfig{
		...
		DenyHandler: func(c echo.Context, identifier string, err error) error {
			return c.String(http.StatusTooManyRequests, "Your request has been rate limited. Please try again later. Contact security@gndr.app if you believe this was a mistake.\n")
		},
	}))
	...
	// enable some special endpoints for the "canonical" deployment (gndr.app). not having these enabled should *not* impact regular operation
...
func (srv *Server) Download(c echo.Context) error {
	...
	if strings.Contains(ua, "Android") {
		return c.Redirect(http.StatusFound, "https://play.google.com/store/apps/details?id=xyz.ganderweb.app")
	}
