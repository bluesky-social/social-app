...
ENV SENTRY_AUTH_TOKEN=${SENTRY_AUTH_TOKEN:-dev}

RUN echo "SENTRY_AUTH_TOKEN=$SENTRY_AUTH_TOKEN"
...
RUN \. "$NVM_DIR/nvm.sh" && \
  ...
  yarn && \
  yarn intl:build && \
  EXPO_PUBLIC_BUNDLE_IDENTIFIER=$EXPO_PUBLIC_BUNDLE_IDENTIFIER EXPO_PUBLIC_BUNDLE_DATE=$() SENTRY_AUTH_TOKEN=$SENTRY_AUTH_TOKEN yarn build-web
