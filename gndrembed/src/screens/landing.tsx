...
function LandingPage() {
  ...
  const [thread, setThread] = useState<AppGndrFeedDefs.ThreadViewPost | null>(
    null,
  )
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        if (uri) {
          if (uri.startsWith('at://')) {
            ...
            try {
              ...
              if (!urlp.hostname.endsWith('gndr.app')) {
              ...
              console.log(err)
              throw new Error('Invalid Gander URL')
            }
          }
        }
        ...
        console.error(err)
        setError(err instanceof Error ? err.message : 'Invalid Gander URL')
      } finally {
        setLoading(false)
      }
    })()
  ...
  return (
    <main className="w-full min-h-screen flex flex-col items-center gap-8 py-14 px-4 md:pt-32 dark:bg-dimmedBgDarken dark:text-slate-200">
      ...
      <h1 className="text-4xl font-bold text-center">Embed a Gander Post</h1>
...
function Snippet({
  ...
  const snippet = useMemo(() => {
    ...
    if (!AppGndrFeedPost.isRecord(record)) {
      return ''
    }
...
function toShareUrl(path: string) {
  return `https://gndr.app${path}?ref_src=embed`
}
