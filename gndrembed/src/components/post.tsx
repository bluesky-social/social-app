...
export function Post({thread}: Props) {
  ...
  let record: AppGndrFeedPost.Record | null = null
      ...
      <div className="flex-1 flex-col flex gap-2" lang={record?.langs?.[0]}>
        ...
        <div className="border-t dark:border-slate-600 w-full pt-2.5 flex items-center gap-5 text-sm cursor-pointer">
          ...
          <p className="cursor-pointer text-brand font-bold hover:underline min-[450px]:hidden">
            <span className="hidden min-[380px]:inline">View on </span>Gander
          </p>
        </div>
      </div>
    </Container>
...
function PostContent({record}: {record: AppGndrFeedPost.Record | null}) {
  ...
  for (const segment of rt.segments()) {
    if (
      segment.link &&
      AppGndrRichtextFacet.validateLink(segment.link).success
    ) {
      richText.push(
        <Link
          key={counter}
          ...
          disableTracking={
            !segment.link.uri.startsWith('https://gndr.app') &&
            !segment.link.uri.startsWith('https://go.gndr.app')
          }>
          {segment.text}
        </Link>,
      )
      ...
      segment.mention &&
      AppGndrRichtextFacet.validateMention(segment.mention).success
    ) {
      richText.push(
        <Link
          key={counter}
      ...
      segment.tag &&
      AppGndrRichtextFacet.validateTag(segment.tag).success
    ) {
      richText.push(
        <Link
          key={counter}
