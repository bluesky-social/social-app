import { AppGndrEmbedExternal, AppGndrEmbedImages, AppGndrEmbedRecord, AppGndrEmbedRecordWithMedia, AppGndrEmbedVideo, AppGndrFeedDefs, AppGndrFeedPost, AppGndrGraphDefs, AppGndrGraphStarterpack, AppGndrLabelerDefs,  } from '@gander-social-atproto/api'
...
export function Embed({
  ...
  content: AppGndrFeedDefs.PostView['embed']
  ...
  try {
    ...
    if (AppGndrEmbedRecord.isView(content)) {
      ...
      if (AppGndrEmbedRecord.isViewRecord(record)) {
        ...
        let text
        if (AppGndrFeedPost.isRecord(record.value)) {
      ...
      // Case 3.2: List
      if (AppGndrGraphDefs.isListView(record)) {
        return (
          <GenericWithImageEmbed
            ...
            subtitle={
              record.purpose === AppGndrGraphDefs.MODLIST
      ...
      // Case 3.3: Feed
      if (AppGndrFeedDefs.isGeneratorView(record)) {
        return (
          <GenericWithImageEmbed
            image={record.avatar}
      ...
      // Case 3.4: Labeler
      if (AppGndrLabelerDefs.isLabelerView(record)) {
        // Embed type does not exist in the app, so show nothing
        return null
      }

      // Case 3.5: Starter pack
      if (AppGndrGraphDefs.isStarterPackViewBasic(record)) {
      ...
      if (AppGndrEmbedRecord.isViewDetached(record)) {
    ...
    // Case 4: Video
    if (AppGndrEmbedVideo.isView(content)) {
      return <VideoEmbed content={content} />
    }

    // Case 5: Record with media
    if (
      AppGndrEmbedRecordWithMedia.isView(content) &&
      AppGndrEmbedRecord.isViewRecord(content.record.record)
    ) {
      return (
...
function ImageEmbed({
  ...
  content: AppGndrEmbedImages.View
  labelInfo?: string
}) {
  if (labelInfo) {
...
function ExternalEmbed({
  ...
  content: AppGndrEmbedExternal.View
  labelInfo?: string
}) {
...
function VideoEmbed({content}: {content: AppGndrEmbedVideo.View}) {
  let aspectRatio = 1
...
function StarterPackEmbed({
  ...
  content: AppGndrGraphDefs.StarterPackViewBasic
}) {
  if (!AppGndrGraphStarterpack.isRecord(content.record)) {
    return null
  }
...
function getStarterPackImage(starterPack: AppGndrGraphDefs.StarterPackView) {
  const rkey = getRkey({uri: starterPack.uri})
  return `https://ogcard.cdn.gndr.app/start/${starterPack.creator.did}/${rkey}`
}

function getStarterPackHref(
  starterPack: AppGndrGraphDefs.StarterPackViewBasic,
) {
  const rkey = getRkey({uri: starterPack.uri})
